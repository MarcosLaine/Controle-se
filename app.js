...
class ControleSeApp {
        ...
        transactions.forEach(transaction => {
            ...
            item.innerHTML = `
                ...
                <div class="transaction-actions">
                    ...
                    <button class="btn-icon btn-danger delete-transaction-btn" data-transaction-id="${transaction.id}" data-transaction-type="${transaction.type}" title="Excluir">
            ...
            // Adiciona event listener ao botão de exclusão
            const deleteBtn = item.querySelector('.delete-transaction-btn');
            if (deleteBtn) {
                deleteBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    const transactionId = parseInt(deleteBtn.getAttribute('data-transaction-id'));
                    const transactionType = deleteBtn.getAttribute('data-transaction-type');
                    this.deleteTransaction(transactionId, transactionType);
                });
            }
        });
    }
    ...
    showAddExpenseModal() {
        Promise.all([this.loadCategories(), this.loadAccounts()]).then(() => {
            ...
            const accounts = this.getAccountsForSelect(true); // Exclui contas de investimento
    ...
    async deleteTransaction(id, type) {
        const transactionType = type === 'income' ? 'receita' : 'gasto';
        const confirmMessage = `Tem certeza que deseja excluir esta ${transactionType}?`;
        ...
        try {
            ...
            const response = await this.apiCall(`${endpoint}?id=${id}`, 'DELETE');
            
            if (response.success) {
                this.showToast(`${transactionType.charAt(0).toUpperCase() + transactionType.slice(1)} excluída com sucesso!`, 'success');
                this.loadTransactions();
                this.loadAccounts(); // Recarrega contas para atualizar saldo
                this.updateOverview();
            } else {
                this.showToast(response.message || `Erro ao excluir ${transactionType}`, 'error');
            }
        } catch (error) {
            this.showToast('Erro de conexão', 'error');
    ...
    async apiCall(endpoint, method = 'GET', data = null) {
        ...
        console.log('apiCall - Method:', method, 'URL:', url, 'Options:', options);
        const response = await fetch(url, options);
        
        if (!response.ok) {
            console.error('apiCall - Response not OK:', response.status, response.statusText);
        }
    ...
    getAccountsForSelect(excludeInvestment = false) {
        ...
        this.accountsCache.forEach(account => {
            ...
            options += `<option value="${account.idConta}">${account.nome}${tipoDisplay}</option>`;
        });
        return options;
    }
...
<body>
    ...
    async editInvestment(investmentId) {
        try {
            ...
            // Busca o investimento específico (pode estar em qualquer grupo)
            let investment = null;
            for (const inv of response.data) {
                if (inv.idInvestimento === investmentId) {
                    investment = inv;
                    break;
                }
            }
            ...
            const content = `
                <form id="edit-investment-form">
                    ...
                    <div class="form-group">
                        <label for="edit-investment-asset-name">Nome Completo do Ativo (opcional)</label>
                        <input type="text" id="edit-investment-asset-name" value="${investment.nomeAtivo || ''}" placeholder="Ex: Itaú Unibanco, Apple Inc., Bitcoin">
                        <small style="color: var(--neutral-600); margin-top: 4px; display: block;">
                            Nome completo do ativo para melhor identificação
                        </small>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="edit-investment-price" value="${investment.precoAporte}" min="0.01" step="0.01" placeholder="Deixe vazio para buscar automaticamente">
                        <small style="color: var(--neutral-600); margin-top: 4px; display: block;">
    ...
    async handleUpdateInvestment(e, investmentId) {
        ...
        const assetName = document.getElementById('edit-investment-asset-name').value.trim() || null;
